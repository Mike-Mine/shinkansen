    ...
    <div class="p-6 flex space-x-2">
        ...
        <div class="flex-1">
            <div class="flex justify-between items-center">
                <div>
                    <span class="text-gray-800">{{ chatMessage.user.name }}{{ chatMessage.user.deleted_at ? ' (Unlicensed)' : '' }}</span>
                ...
                <Dropdown v-if="chatMessage.user.id === $page.props.auth.user.id || can.manageChatMessages">
                    ...
                    <template #content>
                        ...
                        <DropdownLink as="button" :href="route('chat.destroy', chatMessage.id)" method="delete">
                            Delete
                        </DropdownLink>
                    </template>
                </Dropdown>
            </div>
            <form v-if="editing" @submit.prevent="form.put(route('chat.update', chatMessage.id), { onSuccess: () => editing = false })">
            ...
            <p v-else class="mt-4 text-lg text-gray-900">{{ chatMessage.message }}</p>
        </div>
    </div>
</template>